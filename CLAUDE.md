...
- ID 19 : `DEPENSE - Rbt Dépôt garantie` (caution remboursée) — constante `TYPE_REMBOURSEMENT_CAUTION` dans `principale/services.py` (juillet 2026, remplace une détection par texte dans le nom du type qui existait avant dans `ajouter_transaction`/`modifier_transaction`/`supprimer_transaction`)
...
### Cautions
- `Bien.montant_caution` = montant de référence **attendu** (source de vérité pour calculer un écart/une créance : ce qui est dû)
- `LocationBien.montant_caution` = montant historique (ne plus utiliser comme référence)
- ⚠️ **Montant réellement détenu/encaissé = TOUJOURS la somme des transactions `type_transaction_id=18` (`TYPE_DEPOT_GARANTIE`)**, jamais `Bien.montant_caution` ni `LocationBien.montant_caution`. Ces deux champs ne représentent que le montant *attendu*, pas ce qui a été réellement payé (un locataire peut avoir versé un montant différent). Piège rencontré juillet 2026 : le Bilan comptable détaillé et l'export "État des cautions" affichaient des totaux différents pour la même chose car l'un sommait les transactions réelles et l'autre utilisait `Bien.montant_caution` — corrigé pour que tout calcul de "total des cautions détenues" passe par la somme des transactions.
- ⚠️ `Locataire.montant_caution` / `Locataire.date_versement_caution` / `Locataire.date_restitution_caution` sont des champs **totalement morts** (jamais renseignés depuis la bascule vers la saisie par transaction) — ne jamais s'en servir comme source de données, uniquement `LocationBien.date_versement_caution`/`date_restitution_caution` (mis à jour automatiquement) et les transactions pour le montant.
- La saisie manuelle de caution est supprimée — tout passe par les transactions
- `LocationBien.date_versement_caution` = mise à jour automatique à la saisie d'une transaction `TYPE_DEPOT_GARANTIE`
- `LocationBien.date_restitution_caution` = mise à jour automatique à la saisie d'une transaction `TYPE_REMBOURSEMENT_CAUTION`
...
- **Juillet 2026 — analyse de code et fiabilité des cautions** : passe d'analyse globale du code (robustesse, doublons, perf). Corrections principales : le total des cautions du Bilan comptable détaillé était toujours à 0€ (basé sur `Locataire.montant_caution`, champ mort) ; l'export "État des cautions" utilisait `Bien.montant_caution`/`LocationBien.montant_caution` (montant de référence) au lieu de la somme réelle des transactions, ce qui pouvait afficher un total différent du Bilan comptable pour le même concept (voir §10 Cautions) ; l'export PDF/Excel des créances excluait à tort les locataires totalement partis avec un solde résiduel (désormais aligné sur `locations_ouvertes`, comme l'onglet Créances) ; les quittances n'appliquaient pas les révisions de loyer/prorata premier mois dans le rappel des montants attendus ; ajout de vérifications SCI manquantes sur la gestion des ordures ménagères ; suppression de `principale/utils.py` (code mort). **Leçon** : plusieurs endroits du code calculaient "le montant de la caution" de façons différentes (transactions réelles vs champs de référence) sans que ce soit visible avant une comparaison chiffre à chiffre — bien vérifier qu'un même concept métier est toujours calculé de la même façon partout où il apparaît, pas seulement corriger un endroit isolément.
- **Juillet 2026 — comparer la base locale et la base de production** : technique utilisée pour diagnostiquer un écart de chiffres entre local et production qui n'était pas un bug de code mais un désalignement des données (David travaille parfois sur les deux environnements sans toujours resynchroniser). Procédure : PythonAnywhere → onglet **Files** → naviguer jusqu'à `Gestion locative/gestion_locations/db.sqlite3` → cliquer sur le fichier → bouton **Download**. En local (Git Bash, serveur arrêté) : mettre de côté l'ancienne base (`mv db.sqlite3 db.sqlite3.local_avant_comparaison`) puis copier le fichier téléchargé à sa place (`mv ~/Downloads/db.sqlite3 db.sqlite3`, dossier `Downloads` en anglais même sur Windows en français) avant de relancer `python manage.py runserver`. Utile pour rejouer un calcul avec des données identiques des deux côtés avant de conclure à un bug de code. La sauvegarde automatique de 2h (`backups/latest_backup.json`) est une alternative si un accès PythonAnywhere n'est pas possible dans l'immédiat, mais elle peut avoir jusqu'à 24h de retard sur la production.
